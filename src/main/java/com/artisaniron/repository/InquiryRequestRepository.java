...
public class InquiryRequestRepository {
    ...
    public InquiryRequest save(InquiryRequest inquiry) {
        ...
        inquiry.setId(key);
        firebaseDatabase.getReference(COLLECTION).child(key).setValue(inquiry, (error, ref) -> {});
        return inquiry;
    }

    public void markAsRead(String id) {
        firebaseDatabase.getReference(COLLECTION).child(id).child("read").setValue(true, (error, ref) -> {});
    }

    public void updateStatus(String id, String status) {
        firebaseDatabase.getReference(COLLECTION).child(id).child("status").setValue(status, (error, ref) -> {});
    }
}
